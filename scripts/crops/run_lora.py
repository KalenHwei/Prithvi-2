...
# NEW: Import necessary components from the PEFT library
...
def get_args():
    ...
    # NEW: Add LoRA specific arguments
...
# NEW: Add a helper function to display the number of trainable parameters
...
def main():
    ...
    # Initialize the logger with the custom experiment name.
    # The logger will automatically create the full path: base_log_dir/name/version
    # For example: finetuned_checkpoints/logs/multi-temporal-crop/your_custom_name
    ...
    # The checkpoint callback now saves directly into the logger's specific experiment directory.
    # We get this path from the `logger.log_dir` attribute.
    ...
    model = SemanticSegmentationTask(
        ...
        ignore_index=-1,
        # MODIFIED: Pass the argument to the task.
        # The task will handle freezing internally. When using LoRA, the base model is frozen by peft.
        freeze_backbone=cfg.freeze_backbone, 
        freeze_decoder=False,
        model_factory="EncoderDecoderFactory",
    )

    # NEW: Conditionally apply LoRA to the backbone
    # This happens only if you want to train the backbone (`freeze_backbone` is False) and have enabled LoRA.
    if not cfg.freeze_backbone and cfg.use_lora:
        print("INFO: LoRA is enabled. Applying LoRA to the backbone...")
        
        # Define the LoRA configuration.
        # For Vision Transformers like Prithvi, you typically target the query, key, value, and projection layers.
        # 'qkv' is a common name for the combined query/key/value projection layer.
        # 'proj' is common for the output projection in the attention block.
        ...
        # The actual model is stored in the `model` attribute of the Lightning task
